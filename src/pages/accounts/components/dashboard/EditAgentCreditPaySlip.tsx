...
interface EditAgentCreditPaySlipProps {
  onClose: () => void;
  onSave: (data: DataProps) => void;
  initialData: InitialDataProps;
}

const EditAgentCreditPaySlip: React.FC<EditAgentCreditPaySlipProps> = ({
  onClose,
  onSave,
  initialData,
}) => {
  ...
  return (
    <div className="flex flex-col gap-6 px-8 py-6 bg-grey-aw-50 rounded-xs shadow-lg max-w-[800px] max-h-[600px] overflow-auto custom-scrollbar">
      ...
      <GroupField
        ...
        error={false}
        options={initialData.creditNoteInvoiceNumbers.map((item) => ({
          label: item,
          value: item,
        }))}
        isMulti
...
export default EditAgentCreditPaySlip;
