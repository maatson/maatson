...
const CarrierInvoiceDetails: React.FC = () => {
  const { id } = useParams();
  ...
  return (
    <div className="bg-grey-aw-50 flex flex-col gap-6 px-8 py-6 rounde-xs shadow-lg">
      <div className="flex justify-end gap-4">
        <NavLink to={`/accounts/carrier-invoice-export/edit/${id}`}>
          <NeutralBlueButton
            label={"Edit"}
            size={"m"}
            variant={"primary"}
      ...
      <div className="flex flex-col gap-4">
        <div className="grid gap-4 grid-cols-5">
          ...
          <AccountsModel
            ...
            parentStyle="flex-col"
          />
          <AccountsModel label={"LINE NO"} value={"3"} parentStyle="flex-col" />
        </div>
        <div className="grid gap-4 grid-cols-5">
          <AccountsModel
            label={"VESSEL "}
      ...
      <div className="flex flex-col gap-2">
        ...
        <p className="text-sm text-grey-ab">
          Confirmation of successful collection Pending or delayed collections
          with reasons Issues encountered during collection (e.g., damaged
          goods, incomplete payment) Follow-up actions required Special
          instructions or notes from the collector or supervisor
        </p>
      </div>
...
export default CarrierInvoiceDetails;
