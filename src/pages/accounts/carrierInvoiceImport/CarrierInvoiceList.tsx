...
interface RowData {
  ...
  carrierName: string;
  carrierInvoiceStatus: string | React.ReactNode;
...
const CarrierInvoiceList: React.FC = () => {
  ...
  const createData = (items: any) => {
    const { id, carrierInvoiceStatus, paymentStatus, invoiceStatus } = items;

    const carrierInvoiceStatusValue = (
      <>
        {carrierInvoiceStatus.toLowerCase() === "pending" && (
          <SecondaryChip
            label={carrierInvoiceStatus}
            size={"m"}
            variant={"outline"}
          />
        )}
        {carrierInvoiceStatus.toLowerCase() === "completed" && (
          <SuccessChip
            label={carrierInvoiceStatus}
            size={"m"}
            variant={"outline"}
    ...
    const actions = (
      <>
        <Link to={`/accounts/carrier-invoice-import/view/${id}`}>
          <PrimaryButton
            label={"View"}
            size={"s"}
            variant={"primary"}
    ...
    const updatedData = {
      ...
      carrierInvoiceStatus: carrierInvoiceStatusValue,
  ...
  const data = [
    ...
    },
    {
      ...
      carrierName: "Dimas Kamal",
      carrierInvoiceStatus: "Completed",
      paymentStatus: "Paid",
...
export default CarrierInvoiceList;
