...
interface RowData {
  ...
  cargoHandoverStatus: React.ReactNode;
  cargoHandoverUpdate: string;
...
const CargoHandoverUpdate: React.FC = () => {
  ...
  const createData = (items: any) => {
    const { id, cargoHandoverStatus } = items;

    const cargoHandoverStatusValue = (
      <>
        {cargoHandoverStatus.toLowerCase() === "processing" ? (
          <WarningChip label={cargoHandoverStatus} size={"m"} variant={"mix"} />
        ) : (
          <SuccessChip label={cargoHandoverStatus} size={"m"} variant={"mix"} />
        )}
      </>
    );
    const actions = (
    ...
    const updatedData = {
      ...
      cargoHandoverStatus: cargoHandoverStatusValue,
      cargoHandoverUpdate: items?.cargoHandoverUpdate || "-",
      action: actions,
    };
    return updatedData;
  };
...
export default CargoHandoverUpdate;
