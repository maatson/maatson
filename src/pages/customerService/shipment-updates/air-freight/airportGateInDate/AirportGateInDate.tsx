...
interface RowData {
  ...
  airportGateInStatus: React.ReactNode;
...
const AirportGateInDate: React.FC = () => {
  ...
  const createData = (items: any) => {
    const { id, airportGateInStatus } = items;

    const airportGateInStatusValue = (
      <>
        {airportGateInStatus.toLowerCase() === "processing" ? (
          <WarningChip label={airportGateInStatus} size={"m"} variant={"mix"} />
        ) : (
          <SuccessChip label={airportGateInStatus} size={"m"} variant={"mix"} />
        )}
      </>
    );
    const actions = (
      <Link
        to={`/shipment-updates/air-freight/airport-gatein-date/view/${id + 1}`}
    ...
    const updatedData = {
      ...
      airportGateInStatus: airportGateInStatusValue,
...
export default AirportGateInDate;
