import React, { useCallback, useEffect, useState } from "react";
import {
  DeleteIcon,
  JsonFileIcon,
  SearchIcon,
} from "../../../../../components/icons/Icons";
import NeutralBlueButton from "../../../../../components/buttons/NeutralBlueButton";
import GroupField from "../../../../../components/groupField/GroupField";
import { MenuItem, Select, SelectChangeEvent } from "@mui/material";
import CustomPagination from "../../../../../components/pagination/CustomPagination";
import CustomTable from "../../../../../components/table/CustomTable";
import DefaultDp from "/images/defaultProfilePic.png";
import BulkScheduleCard from "../../components/cards/BulkScheduleCard";

interface RowData {
  id: string | number;
  carrierName: string | React.ReactNode;
  updatedPerson: string | React.ReactNode;
  updatedDate: string | React.ReactNode;
  action: React.ReactNode; // If you have any actions like buttons or components
}

const columns: any[] = [
  {
    id: "carrierName",
    label: "Carrier Name",
    minWidth: "180px",
  },
  { id: "updatedDate", label: "Updated Date" },
  { id: "updatedPerson", label: "Updated Person", minWidth: "180px" },
  {
    id: "action",
    label: "Action",
    align: "center",
  },
];

const BulkScheduleUpdates: React.FC = () => {
  const [currentPage, setCurrentPage] = useState<number>(1);
  const [rows, setRows] = useState<RowData[]>([]);
  const [itemsPerPage, setItemsPerPage] = React.useState(5);
  const [selectedRows, setSelectedRows] = useState<(string | number)[]>([]); // Track selected row ids
  const [isBulkSchedule, setBulkSchedule] = useState<boolean>(false);
  // Handle the change in checked rows
  const handleCheckedRowsChange = (newCheckedRows: (string | number)[]) => {
    setSelectedRows(newCheckedRows);
  };
  console.log(selectedRows, "selected Rows");
  const handleItemsPerPageChange = useCallback(
    (event: SelectChangeEvent<number>) => {
      setItemsPerPage(Number(event.target.value));
      setCurrentPage(1);
    },
    []
  );

  const handlechangePage = (
    _event: React.ChangeEvent<unknown>,
    page: number
  ) => {
    setCurrentPage(page);
  };

  const createData = (items: any) => {
    const {
      id,
      carrierName,
      updatedPerson,

      updatedDate,
    } = items;
    // Define your actions or any other custom logic you need for each row
    const actions = (
      <div className="rounded flex justify-center items-center bg-red w-8 h-8 mx-auto">
        <DeleteIcon size={16} color="#fdfdfd" />
      </div>
    );

    const carrierNameValue = (
      <div className="px-2 py-1 flex items-center gap-2 text-sm">
        <div className="min-h-10 min-w-10">
          <img
            src={DefaultDp}
            alt="carrierImage"
            className="object-fill  h-10 w-10"
          />
        </div>

        <p className="w-full"> {carrierName || "Nil"}</p>
      </div>
    );
    const updatedPersonValue = (
      <div className="px-2 py-1 flex items-center gap-2 text-sm">
        <div className="min-h-10 min-w-10">
          <img
            src={DefaultDp}
            alt="carrierImage"
            className="object-fill  h-10 w-10"
          />
        </div>

        <p className="w-full"> {updatedPerson || "Nil"}</p>
      </div>
    );

    // Ensure all columns have a value (or a default) for each row.
    const updatedData = {
      id: id,
      updatedDate: updatedDate || "N/A", // Default value if not available
      carrierName: carrierNameValue, // Default value if not available
      updatedPerson: updatedPersonValue, // Default value if not available

      action: actions, // Actions will remain the same as you defined
    };

    return updatedData;
  };

  const data = [
    {
      carrierName: "John Doe",
      updatedPerson: "Emp001",
      updatedDate: "2025-02-01",
    },
    {
      carrierName: "",
      updatedPerson: "Emp002",
      updatedDate: "2025-02-01",
    },
    {
      carrierName: "Pavi ",
      updatedPerson: "Emp003",
      updatedDate: "2025-02-01",
    },
    {
      carrierName: "Janani ",
      updatedPerson: "junior Developer",
      updatedDate: "",
    },
  ];
  // Memoize fetchData function with useCallback
  const fetchData = useCallback(() => {
    const arr = data.map((items, index) => {
      return createData({ ...items, id: index }); // Ensure createData returns the transformed data
    });
    setRows(arr); // Set the rows with the updated data
  }, []); // Empty dependency array ensures this function is only created once

  useEffect(() => {
    fetchData(); // Call fetchData when the component mounts
  }, [fetchData]); // Only re-run fetchData if fetchData changes

  return (
    <>
      {isBulkSchedule && (
        <BulkScheduleCard
          isSeaFreight={true}
          onCancel={() => setBulkSchedule(false)}
          onUpdate={() => setBulkSchedule(false)}
        />
      )}
      <div className="p-3 flex justify-between items-center text-grey-ab-900 border-b border-grey-ab-100 ">
        <div className="flex gap-3 items-center">
          <p className="text-lg font-semibold w-full">Bulk Schedule Updates</p>{" "}
          <GroupField
            label={""}
            type={""}
            placeholder={"Search"}
            name={""}
            value={""}
            onChange={() => {}}
            parentStyle="max-w-[320px] w-full min-w-[180px]"
            error={false}
            rightIcon={<SearchIcon />}
            errorMessage={""}
          />
        </div>
        <div onClick={() => setBulkSchedule(true)}>
          <NeutralBlueButton
            label={"Import"}
            size={""}
            variant={""}
            leftIcon={<JsonFileIcon color="#fdfdfd" />}
          />
        </div>
      </div>
      <div className="px-3 py-2">
        <CustomTable
          columns={columns}
          rows={rows}
          isCheckbox={true}
          onCheckedRowsChange={handleCheckedRowsChange} // Pass the handler here
        />
      </div>
      <div className="px-3 py-4 flex justify-between items-center">
        <div className="text-xs text-grey-ab-200">
          Showing {currentPage * itemsPerPage - itemsPerPage + 1} to{" "}
          {currentPage *
            (itemsPerPage > rows.length ? rows.length : itemsPerPage)}{" "}
          of {rows.length} Entries
        </div>

        <CustomPagination
          totalItems={rows.length}
          itemsPerPage={itemsPerPage}
          currentPage={currentPage}
          handlePageChange={handlechangePage}
        />

        <div className="flex gap-4 items-center">
          <p className="text-xs text-grey-ab-300">Items Per Page</p>
          <Select
            value={itemsPerPage}
            onChange={handleItemsPerPageChange}
            size="small"
            sx={{
              fontSize: "12px",
              fontWeight: "700",
              color: "#121212",
              padding: "0px 4px",
              borderRadius: "4px",
            }}
          >
            {[5, 10, 15, 20, 25].map((option) => (
              <MenuItem key={option} value={option}>
                {option}
              </MenuItem>
            ))}
          </Select>
        </div>
      </div>
    </>
  );
};

export default BulkScheduleUpdates;
