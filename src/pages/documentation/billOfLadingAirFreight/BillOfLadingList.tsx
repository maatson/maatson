...
const BillOfLadingList: React.FC = () => {
  ...
  const createData = (items: any) => {
    const { id, blStatus } = items;

    const blStatusValue = (
      <>
        {blStatus.toLowerCase() === "pending" && (
          <SecondaryChip label={blStatus} size={"m"} variant={"mix"} />
        )}
        {blStatus.toLowerCase() === "created" && (
          <BlueChip label={blStatus} size={"m"} variant={"mix"} />
        )}
        {blStatus.toLowerCase() === "printed" && (
          <SuccessChip label={blStatus} size={"m"} variant={"mix"} />
        )}
      </>
    );
    const actions = (
      <>
        <Link to={`/bill-of-lading/air-freight/view/${id}`}>
          <NeutralBlueButton
            label={"View"}
            size={"s"}
            variant={"primary"}
            rightIcon={<DocumentIcon size={16} color="#ffffff" />}
          />
        </Link>
      </>
    );
